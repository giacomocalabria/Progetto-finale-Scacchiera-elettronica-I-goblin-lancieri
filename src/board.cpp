...
{    
    ...
    if(is_king(p) && !is_check(p->get_player()) && is_castling(from, to))
    {
        ...
        if(from.col > to.col) // va a sinistra
        {
            ...
            // Controllo che tra tra la torre e il re non ci sia nessun pezzo. Se ci sono pezzi allora non è possibile eseguire l'arrocco.
            ...
            // Controllo se nella posizione intermedia c'è scacco al re: non ci può essere scacco al re in tale posizione.
...
// Controlla se può eseguire l'en passant
...
bool board::can_do_legal_move(player_id pl) const
{
    ...
    for (auto p : board_matrix)
    {
        ...
        for (auto pos : possible_pos)
            if (p->can_move_to(pos, board_matrix)) return true; // Se la pedina può muoversi verso tale posizione allora ritorna true
    }
